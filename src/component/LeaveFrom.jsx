...
const LeaveForm = ({ user, type }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.fullname) return alert("⚠️ กรุณาระบุรายชื่อผู้ลา");
    ...
    try {
        ...
        if (res.ok) {
            alert("✅ บันทึกคำขอลาพักผ่อนสำเร็จ!");
  ...
  return (
    <div className="min-h-screen pt-8 pb-12 px-4 flex justify-center bg-gray-50 font-sarabun">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden mx-auto border border-gray-100">
        ...
        {/* Header Section */}
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 p-8 text-white shadow-inner">
          <div className="flex items-center gap-4">
            <span className="text-5xl drop-shadow-md">🏖️</span>
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight">บันทึกคำขอลาพักผ่อน</h2>
              <p className="text-blue-100 text-sm mt-1 font-medium">ค้นหาและดึงข้อมูลข้าราชการตำรวจจากฐานข้อมูล</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          ...
          <div className="relative mb-10" ref={dropdownRef}>
            ...
            <div className="relative flex items-center">
                ...
                <input 
                    type="text" 
                    className="w-full py-3.5 pr-4 pl-12 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 shadow-sm"
            ...
            {isDropdownOpen && (
                <ul className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl mt-2 shadow-2xl z-50 max-h-64 overflow-y-auto list-none p-0 divide-y divide-gray-100">
                    {isLoading ? (
                        ...
                        filteredUsers.map((u) => (
                            <li key={u.id} className="p-4 cursor-pointer hover:bg-blue-50 transition-colors group" onClick={() => handleSelectUser(u)}>
                                <span className="block font-bold text-gray-800 group-hover:text-blue-700">{u.rank} {u.name}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
            <div>
                ...
                <input name="rank" className="w-full p-3.5 border border-gray-200 rounded-xl outline-none bg-gray-50 text-blue-700 font-bold cursor-not-allowed" value={formData.rank} onChange={handleChange} placeholder="ยศ" readOnly />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
                ...
                <input 
                    name="duty" 
                    className="w-full p-3.5 border-2 border-blue-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20" 
                    value={formData.duty} 
                ...
                {leaderPreview && leaderPreview.cmd_name && leaderPreview.cmd_name !== '..........................' && (
                    <div className="mt-3 p-4 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-800 flex items-start gap-3 shadow-sm">
                        <span className="text-xl">👤</span>
                        <div>
                            <strong className="block mb-0.5 text-blue-900">เสนอต่อ (ผู้พิจารณาคำขอ):</strong> 
            ...
            <div className="md:col-span-2">
                ...
                <input name="phone" className="w-full p-3.5 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20" value={formData.phone} onChange={handleChange} placeholder="เบอร์โทรศัพท์..." />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
             <div>
                ...
                <input type="date" name="startDate" className="w-full p-3.5 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20" onChange={handleChange} />
             </div>
             <div>
                <label className="block text-sm font-bold text-gray-600 mb-2">ถึงวันที่ <span className="text-red-500">*</span></label>
                <input type="date" name="endDate" className="w-full p-3.5 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20" onChange={handleChange} />
             </div>
             <div className="md:col-span-2">
                <label className="block text-sm font-bold text-gray-600 mb-2">รวม (วัน)</label>
                <input type="text" className="w-full p-3.5 border-2 border-blue-200 rounded-xl outline-none bg-blue-50 text-blue-700 font-extrabold text-center text-xl cursor-not-allowed" value={formData.totalDays} readOnly />
             </div>

             {/* หากประเภทการลาเปลี่ยน จะมีช่องให้ใส่เหตุผล */}
             {formData.leaveType !== 'ลาพักผ่อน' && (
               <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-600 mb-2">เหตุผลการลา</label>
                  <textarea name="reason" rows="3" className="w-full p-3.5 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-700 font-medium focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20" placeholder="ระบุสาเหตุ..." onChange={handleChange}></textarea>
               </div>
             )}
          </div>

          <button onClick={handleSubmit} disabled={isSubmitting} className="w-full p-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl mt-8 transition-all shadow-lg hover:shadow-blue-500/30 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 text-lg flex justify-center items-center gap-2">
            {isSubmitting ? (
              <>
